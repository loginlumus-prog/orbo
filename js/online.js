...
HR.Online = {
  ...
  // rede ruim travava a lista em "Carregando…" para sempre: todo pedido morre em 8 s
  // (o iOS 15 tem AbortController, mas não tem AbortSignal.timeout)
  fetchT(url, opt, ms) {
    const c = new AbortController(), t = setTimeout(() => c.abort(), ms || 8000);
    const o = Object.assign({}, opt || {}); o.signal = c.signal;
    return fetch(url, o).then(r => { clearTimeout(t); return r; }, e => { clearTimeout(t); throw e; });
  },
  ...
  async session() {
    ...
    try {
      let res;
      if (o.auth && o.auth.rt) {
        res = await this.fetchT(B.url + '/auth/v1/token?grant_type=refresh_token', { method: 'POST', headers: this.headers(), body: JSON.stringify({ refresh_token: o.auth.rt }) });
      ...
      if (!res) res = await this.fetchT(B.url + '/auth/v1/signup', { method: 'POST', headers: this.headers(), body: JSON.stringify({ data: { game: 'orbo' } }) });
  ...
  async submit(force) {
    ...
    try {
      ...
      const res = await this.fetchT(B.url + '/rest/v1/' + B.table + '?on_conflict=id', { method: 'POST', headers: Object.assign(this.headers(s.at), { Prefer: 'resolution=merge-duplicates,return=minimal' }), body: JSON.stringify([this.row(s.uid)]) });
      this.cache = {};
      return res.ok;
  ...
  async top(kind) {
    ...
    if (this.enabled()) {
      try {
        ...
        const res = await this.fetchT(B.url + '/rest/v1/' + B.table + '?select=id,name,flag,best,pct,layers,cleared_at&order=' + order + '&limit=50', { headers: this.headers() });
        if (!res.ok) throw new Error('http ' + res.status);
        const rows = await res.json(), uid = this.me().auth && this.me().auth.uid;
        rows.forEach(r => { r.me = r.id === uid; });
        let first = null;
        try { const f = await this.fetchT(B.url + '/rest/v1/' + B.table + '?select=name,flag,cleared_at&layers=eq.11&order=cleared_at.asc&limit=1', { headers: this.headers() }); if (f.ok) first = (await f.json())[0] || null; } catch (_) { /* sem primeiro */ }
        v = { rows, online: true, first };
      } catch (_) { v = this.demo(kind); v.erro = true; }   // tempo esgotado / fora do ar: a tela avisa e oferece tentar de novo
    ...
    if (!v.erro) this.cache[key] = { t: Date.now(), v };   // erro não fica guardado por um minuto
    return v;
  },
  demo(kind) {
...
Object.assign(HR.I18N.pt, {
  ...
  lb_error: 'Não foi possível carregar o ranking agora.', lb_retry: 'Tentar de novo', lb_offline: 'Ranking online ainda não conectado: mostrando uma demonstração.', lb_online: 'Ranking online, aberto para todos. Atualiza ao fim de cada partida.', lb_loading: 'Carregando…',
...
Object.assign(HR.I18N.en, {
  ...
  lb_error: 'Could not load the leaderboard right now.', lb_retry: 'Try again', lb_offline: 'Online leaderboard not connected yet: showing a demo.', lb_online: 'Online leaderboard, open to everyone. Updates after every run.', lb_loading: 'Loading…',
...
Object.assign(HR.I18N.es, {
  ...
  lb_error: 'No se pudo cargar el ranking ahora.', lb_retry: 'Intentar de nuevo', lb_offline: 'Ranking online aún no conectado: mostrando una demostración.', lb_online: 'Ranking online, abierto a todos. Se actualiza al final de cada partida.', lb_loading: 'Cargando…',
